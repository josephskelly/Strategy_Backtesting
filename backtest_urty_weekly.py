"""
URTY Weekly Balance Backtest

Runs the mean reversion strategy on URTY and outputs weekly snapshots:
...
def fetch_urty_closes() -> pd.DataFrame:
    """Fetch closing prices for URTY."""
    print("Fetching URTY data from Yahoo Finance...")
    rows = _yahoo_chart("URTY", range_="30y")
    df = pd.DataFrame(rows).set_index("date")
    df.columns = ["URTY"]
    df = df.sort_index()
    print(f"  URTY: {len(df)} trading days ({df.index[0].date()} to {df.index[-1].date()})")
    return df


def main():
    # Fetch data
    prices_df = fetch_urty_closes()
    ...
    # Save to CSV
    output_path = "urty_weekly_balances.csv"
