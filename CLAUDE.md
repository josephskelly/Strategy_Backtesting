...
- [x] **`sector_trades` naming misleading** (`engine.py:115`) — tracks per-ticker, not per-sector
- [x] **Hardcoded magic numbers** — `$10` min trade, `0.001` position threshold, `252` trading days, `0.25` margin cap
...
- [x] **pytest warning: divide by zero in archived test** (`_archive/test_backtest_engine.py::TestEdgeCases::test_zero_threshold`) — `_archive/backtest_engine.py:124` divides by `z_threshold` without guarding against zero; either fix the guard or remove the archived test from pytest collection
