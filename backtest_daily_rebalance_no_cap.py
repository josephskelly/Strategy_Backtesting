"""
Daily Rebalancing Strategy WITHOUT 25% Per-Trade Margin Cap

This version removes the safety constraint that limits trades to 25% of cash.
Useful for testing what happens if you allow unrestricted position sizing.

Key difference from backtest_daily_rebalance.py:
- Line 120 removed: trade_value = min(trade_value, self.cash * 0.25)
- Now trades can be ANY size up to available cash
"""
...
class DailyRebalanceBacktesterNoCap:
    """
    Daily rebalancing strategy WITHOUT 25% margin cap.

    Allows unrestricted position sizing - trades can be any size.
    """
